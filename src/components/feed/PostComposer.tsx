...
export const PostComposer = ({ onSuccess }: { onSuccess?: () => void }) => {
  ...
  return (
    <motion.div 
      className={`w-full bg-surface border transition-colors duration-300 p-5 ${
        isFocused ? 'border-primary/50 shadow-[0_0_20px_var(--bg-void)]' : 'border-ghost'
      }`}
    >
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="mb-4 bg-void border border-primary/20 p-3 rounded-sm relative group overflow-hidden">
               ...
               {quoteData.imageUrl && (
                 <div className="w-16 h-16 float-right ml-3 mb-1 bg-void border border-ghost overflow-hidden relative z-10">
                   <img src={quoteData.imageUrl} className="w-full h-full object-cover filter grayscale" />
                 </div>
               )}
          ...
          <AnimatePresence>
            {imagePreview && (
              ...
              >
                <img src={imagePreview} alt="Encryption Preview" className="w-full h-auto max-h-[300px] object-cover filter contrast-125 brightness-90 grayscale hover:grayscale-0 transition-all" />
                <button
                  onClick={clearImage}
          ...
          <div className="flex flex-col gap-3 mt-4">
            ...
            <div className="flex items-center justify-between">
              <span className={`text-[10px] font-mono tracking-widest ${
                content.length > MAX_CHARS - 50 ? 'text-danger' : 'text-muted'
              }`}>
